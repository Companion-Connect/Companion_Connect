...
export interface UserProfile {
  ...
  motivational_s: string;
  conversation_c: number;
  lastChatDate: string;
  relationship_le: string;
}
...
export class SyncService {
  ...
  static async syncUserProfile(userId: string): Promise<void> {
    try {
      ...
      const { value: interestsStr } = await Preferences.get({
        key: "profile_interests",
      });
      const interests = interestsStr
        ? JSON.parse(interestsStr)
        : profile.interests;
      const { value: goalsStr } = await Preferences.get({
        key: "challenge_goals",
      });
      ...
      await supabase.from("profiles").upsert({
        ...
        motivational_s: profile.motivational_s,
        conversation_c: profile.conversation_c,
        last_chat_date: profile.lastChatDate,
        relationship_le: profile.relationship_le,
  ...
  static async syncMoodHistory(userId: string): Promise<void> {
    try {
      ...
      const moodHistory: Array<{ date: string; mood: string }> =
        JSON.parse(value);
  ...
  static async syncAllToSupabase(userId: string): Promise<void> {
    ...
    try {
      await Promise.all([
        this.syncUserProfile(userId).catch((e) =>
          console.error("Profile sync error:", e),
        ),
        this.syncChatSettings(userId).catch((e) =>
          console.error("Settings sync error:", e),
        ),
        this.syncUserBadges(userId).catch((e) =>
  ...
  static async loadFromSupabase(userId: string): Promise<void> {
    ...
    const loadOperations = [
      this.loadProfileData(userId).catch((e) =>
        console.error("Profile load error:", e),
      ),
      this.loadChatSettings(userId).catch((e) =>
        console.error("Settings load error:", e),
      ),
      this.loadBadgesData(userId).catch((e) =>
    ...
    try {
      ...
      if (!error && profile) {
        const userProfile: UserProfile = {
          ...
          communication: profile.communication || "",
          motivational_s: profile.motivational_s || "",
          conversation_c: profile.conversation_c || 0,
        ...
        const { value: savedProfile } = await Preferences.get({
          key: "user_profile",
        });
        if (savedProfile) {
          const parsed = JSON.parse(savedProfile);
          console.log(
            "Verification - profile in local storage:",
            parsed.userName,
          );
          ...
          console.error(
            "Verification failed - no profile found in local storage",
          );
        }
      } else {
      ...
      if (progress) {
        await Promise.all([
          Preferences.set({
            ...
            value: progress.streak_count?.toString() || "0",
          }),
          Preferences.set({
            key: "challenge_last_completed",
            value: progress.last_completed || "",
          }),
          Preferences.set({
            key: "weekly_challenge",
            value: JSON.stringify(progress.weekly_challenge || {}),
          }),
          Preferences.set({
            key: "weekly_challenge_week",
            value: progress.challenge_week?.toString() || "1",
          }),
          Preferences.set({
            key: "challenge_completed_count",
            value: progress.completed_count?.toString() || "0",
          }),
          Preferences.set({
            key: "display_badge_id",
            value: progress.display_badge_id || "",
          }),
          Preferences.set({
            key: "lastCompletedChallenge",
            value: JSON.stringify(progress.last_completed_challenge || {}),
          }),
        ]);
      }
    } catch (error) {
